...
import { X, MapPin, Star, LogOut, Settings } from "lucide-react";
...
export default function AccountModal({
  ...
  // Close modal first, then navigate after small delay.
  const navigateAfterClose = (path) => {
    onClose();
    setTimeout(() => {
      navigate(path);
    }, 50); // give React a moment to unmount modal
  };
  ...
  const modal = (
    ...
    >
      ...
      >
          ...
          <div className="flex justify-between items-center bg-pink-50 p-4 rounded-2xl border-2 border-pink-300 mb-4">
            <div>
              <p
                ...
                onClick={() => navigateAfterClose("/profile")}
              >
                {user?.name}
              </p>
            ...
            <button
              onClick={() => navigateAfterClose("/settings")}
        ...
        {/* CONTENT AREA */}
        <div
          className="overflow-y-auto custom-scrollbar border-b-2 border-pink-200"
        ...
        {isEditing ? (
          <>
            ...
            <button
              ...
              className="px-4 py-1 border-2 border-pink-300 rounded-lg font-black text-[10px] text-gray-500"
            >
              CANCEL
            </button>
          </>
